...
const Movie = () => {
    ...
    const [currentMovieDetails, setMovies] = useState()
 ...
 const getDatas = () => {
  ...
  fetch(`https://api.themoviedb.org/3/movie/${id}/credits?api_key=4e44d9029b1270a757cddc766a1bcb63&language=en-US`)
    .then(res => res.json())
    .then(data => setCast(data.cast)) // data.cast is an array of actors
}
    return (
        <div className="movie">
...
<div className="movie__cast">
  {cast && cast.length > 0 ? (
    cast.slice(0, 10).map(actor => (  // show first 10 actors
      <div className="cast__card" key={actor.id}>
        {actor.profile_path ? (
    ...
    <p style={{color:"white"}}>No cast info available</p>
  )}
</div>
